...
relativement facilement un peu de contenu à mon blog. Ensuite, bien qu'il y 
ait déjà énormément de documentation, je trouve qu'elle est parfois complexe
à appréhender. Pour terminer, ça permet, éventuellement, de servir de base,
ou de prérequis de lecture à des articles potentiellement originaux.
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad.ml
module type MONAD =
sig
  (* Le foncteur *)
  type 'a t
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad2.ml
module type MONAD =
sig
  type 'a t
  val pure  : 'a -> 'a t
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad.mli
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad.mli :data-line-start 7
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad.mli :data-line-start 14
module type MONAD =
sig
  type 'a t
  val pure  : 'a -> 'a t
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file monad.ml :data-line-start 8
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file option.ml
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file list.ml
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-file example_div.ml :data-line-number true
let safe_div b a =
  if b = 0 then None
  else Some (a / b)
#+END_SRC
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_div.ml :data-line-start 4
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_div.ml :data-line-start  8
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_list.ml
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_list.ml :data-line-start 7
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file  example_list.ml :data-line-start 9
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file state.mli
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file state.ml
module State (S : sig type t end) :
sig
  type state = S.t
  include MONAD with type 'a t = (state -> 'a * state)
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_state.ml
run (
  pure 0
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-line-number true :data-file example_state.ml :data-line-start 7
run (
  pure 0
  >>= fun index -> put (index + 1)
  >>= fun () -> get
...
#+BEGIN_SRC haskell :data-roe-kind code :data-pellet Haskell :data-file example_do.hs :data-line-number true
...
#+BEGIN_SRC haskell :data-roe-kind code :data-pellet Haskell :data-file example_bind.hs :data-line-number true
cartesian_product :: [(String, Int)]
cartesian_product =
  ["foo", "bar", "foobar"]
  >>= \x -> [1, 3, 9]
...
SQL. Par exemple, pour les *Options* :

#+BEGIN_SRC fsharp :data-roe-kind code :data-pellet Fsharp :data-file example_builder.fs :data-line-number true
// On décrit les opérations valides
type OptionBuilder () = 
...
// On implémente le Workflow
let option = new OptionBuilder()
#+END_SRC

On peut maintenant se servir de src_fsharp{option} comme d'une *do-notation*
spécialisée pour *Option*, par exemple :

#+BEGIN_SRC fsharp :data-roe-kind code :data-pellet Fsharp :data-file example_builder.fs :data-line-number true :data-line-start 14
let safeDivide x y = 
  match y with 
  | 0 -> None 
  | nonNull -> Some (x / nonNull)

let result = option {
  let! a = safeDivide 1 10
  let! b = safeDivide a 3 
  let! c = safeDivide b 0
  return c
}
#+END_SRC
...
#+BEGIN_SRC fsharp :data-roe-kind code :data-pellet Fsharp :data-file example_bind.fs :data-line-number true
let safeDivide x y = 
  match y with 
  | 0 -> None 
  | nonNull -> x / nonNull

// Dans l'assomption que (>>=) existe dans le scope courant
let result = 
  open Option 
  safeDivide 1 10 >>= fun a -> 
  safeDivide a 3  >>= fun b -> 
  safeDivide b 0 >>= fun c -> 
  return c
}
#+END_SRC

De plus, les expressions de calculs permettent d'aller bien plus loin, 
comme l'explique le papier [[https://www.microsoft.com/en-us/research/wp-content/uploads/2016/02/computation-zoo.pdf][The Fsharp computation expression Zoo]].

OCaml, après avoir délègue pendant longtemps les raccourcis syntaxiques pour les 
monades à une [[https://github.com/janestreet/ppx_let][extension de syntaxe externe]] alternative, offre depuis peu 
(src_ocaml{>= 4.08}) la possibilité de [[http://caml.inria.fr/pub/distrib/ocaml-4.08/ocaml-4.08-refman.html#s%3Abinding-operators][de surcharger l'opérateur src_ocaml{let}]]
ce qui permet d'écrire des "programmes monadiques" plus lisibles. Par 
exemple, en reprenant l'exemple du produit cartésien de deux listes : 

#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-file example_let.ml :data-line-number true
let (let*) x f = List.(x >>= f)
let cartesian_product =
  let* x = ["foo"; "bar"; "foobar"] in 
  let* y = [1; 3; 9] in 
  List.return (x, y)
#+END_SRC
...
#+BEGIN_SRC ocaml :data-roe-kind code :data-pellet OCaml :data-file example_bind.ml :data-line-number true
let cartesian_product =
  let open List in
  ["foo"; "bar"; "foobar"] >>= fun x ->
  [1; 3; 9] >>= fun y ->
  List.return (x, y)
#+END_SRC
