...
let start_with base suffix =
  ...
  if len_base = len_suff && len_base = len_suff then
    true
  else if len_base < len_suff then
    false
  else (
    let rec loop i =
      if i >= len_suff then
        true
      else if S.get base i = S.get suffix i then
        loop (succ i)
      else
        false in
    loop 0
...
let end_with base suffix =
  let len_base = S.length base in
  let len_suff = S.length suffix in
  if len_base = len_suff && base = suffix then
    true
  else if len_base < len_suff then
    false
  else (
    ...
    let rec loop i =
      if i >= len_suff then
        true
      else if S.get base (offset_i + i) = S.get suffix i then
        loop (succ i)
      else
        false in
    loop 0
