...
let push_feedback = function
  | Ok (filename, str_log) ->
    Ansi.(
      ...
      |> to_string)
    |> print_endline
  | Error x -> Prompter.prompt_error x
;;
...
let visual_push update log =
  ...
  in
  match up with
  | Ok l -> l |> push_result |> push_feedback
...
let check_day = function
  | None -> Glue.Util.day () |> Validation.from_result
  | Some x -> Timetable.Day.from_string x |> Validation.from_result
;;
...
let record sector duration timecode project label =
  Glue.Ui.ensure_sectors_projects (fun sectors (ctx, projects) ->
      ...
      | Error xs -> Prompter.prompt_errors xs
      | Ok log -> visual_push Shapes.Context.Projects.(ctx.updates) log)
;;
