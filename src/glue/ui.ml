...
let ensure_sectors_projects f =
  ...
  | Error x, Error y -> Prompter.prompt_errors (x @ y)
  | Error x, _ | _, Error x -> Prompter.prompt_errors x
  | Ok sectors, Ok ctx -> f sectors ctx
;;
