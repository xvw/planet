  ...
  | Qexp.Node elts ->
    ...
    let rec aux = function
      ...
      | Ok table, Qexp.Node (key :: value) :: xs ->
        ...
        in
        aux (new_table, xs)
      | _, exp :: _ -> Error (Not_a_valid_node (Qexp.to_string exp))
    in
    aux (Ok table, elts)
  | _ -> Error (No_root_element (Qexp.to_string qexp))
;;
...
let from_qexp f =
  let rf x l =
    let open Qexp in
    match x with
    | Atom key | Tag key | Keyword key | String (_, key) -> f key l
    | q -> Error (Not_a_valid_node (to_string q))
  in
  from_qexp_gen rf
;;
...
module Mapper = struct
  let string = function
    | Qexp.String (_, str) -> Ok str
    | q -> Error [ Mapping_failure ("string", Qexp.to_string q) ]
  ;;
  ...
  let couple f g = function
    | Qexp.Node [ x; y ] ->
      ...
      (fun x y -> x, y) <$> f x <*> g y
    | q -> Error [ Mapping_failure ("couple", Qexp.to_string q) ]
  ;;
  ...
  let triple f g h = function
    ...
    | q -> Error [ Mapping_failure ("triple", Qexp.to_string q) ]
  ;;
end
...
module Fetch = struct
  ...
  let option f table field =
    ...
    | Ok x -> Ok (Some x)
    | Error [ Undefined_field _ ] -> Ok None
    | Error xs -> Error xs
  ;;

  let string table field =
    match Hashtbl.find_opt table field with
    | Some (Some (Qexp.String (_, k))) -> Ok k
    | None -> Error [ Undefined_field field ]
    | _ -> Error [ Invalid_field field ]
  ;;
  ...
  let aux_ziplist f mapper table field =
    ...
    f
      Mapper.(
        ...
        $ token (fun x -> Ok x)
        $ function
        | Node elts -> List.map mapper elts |> Validation.Applicative.sequence
        | _ -> Error [ Invalid_field field ])
      table
      field
  ;;
  ...
  let token mapper table field =
    ...
    | Some (Some x) ->
      (match x with
      | Atom str | String (_, str) | Tag str | Keyword str -> mapper str
      | _ -> Error [ Invalid_field field ])
    | _ -> Error [ Invalid_field field ]
  ;;
