...
val from_qexp_gen :
  (Qexp.t -> Qexp.t list -> ('a * 'b) Result.t) -> Qexp.t -> ('a, 'b) t Result.t
(** Build a [Table.t] from a [Qexp.t] *)

val from_qexp :
  (string -> Qexp.t list -> ('a * 'b) Result.t) -> Qexp.t -> ('a, 'b) t Result.t
...
module Fetch : sig
  ...
  val ziplist_refutable :
    (Qexp.t -> 'a Validation.t) -> (string * 'a list) list t

  val hashtbl :
    (string -> Qexp.t -> ('a * 'b) Validation.t) -> ('a, 'b) Hashtbl.t t
