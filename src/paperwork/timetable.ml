...
module Month = struct
  ...
  let days_in (Month (y, m)) =
    ...
    | Feb -> if Year.is_leap y then 29 else 28
    | _ -> 30
  ;;
    ...
    try
      ...
      Scanf.sscanf str "%2d%2s%2d%!" (fun h flag m ->
          let hr =
            ...
            | "am" -> Ok (if h = 12 then 0 else h)
            | "pm" -> Ok (if h = 12 then h else h + 12)
            | _ -> Error (Unparsable (str ^ ": unknown " ^ flag))
          in
          let* h = hr in
          make h m)
    with
