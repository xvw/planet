...
module Format = struct
  ...
  let from_string str =
    ...
    | "org" | "orgmode" | "org-mode" -> Ok Org
    | x -> Error (Unknown_format x)
  ;;
end
...
let patch_flag str x f =
  match String.lowercase_ascii str with
  | "text" | "plain" | "internal" | "intern" -> Ok (f, Plain x)
  | "file" | "extern" | "external" -> Ok (f, File x)
  | _ -> Error [ Invalid_text_scheme ]
;;
...
let pp ppf (fmt, content) =
  ...
  let kind =
    match content with
    | File k -> Stdlib.Format.sprintf "file://%s" k
    | Plain k -> Stdlib.Format.sprintf "`%s`" k
  in
...
let eq_format left right =
  ...
  | Org, Org | Markdown, Markdown | Raw, Raw -> true
  | _ -> false
;;

let eq_content left right =
  match left, right with
  | File x, File y | Plain x, Plain y -> x = y
  | _ -> false
;;
