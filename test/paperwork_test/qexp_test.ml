...
let parse_empty_data_1 () =
  ...
  | Ok Qexp.(Node []) -> ()
  | Ok qexp -> message (Qexp.to_string qexp)
  | Error err -> message (to_string err)
;;
...
|}
  ...
  | Ok qexp -> parse_message str (Qexp.to_string qexp)
  | Error err -> parse_message str (to_string err)
;;

let parse_failure_1 () =
  match Qexp.from_string ") foo" with
  | Error (Unmatched_character ')') -> ()
  | _ -> failwith "Should fail with unmatched parenthesis"
;;
...
let parse_failure_4 () =
  match Qexp.from_string "foo (bar {)}" with
  | Error (Unmatched_character '{') -> ()
  | _ -> failwith "Should fail with unmatched brace"
;;
...
let suite =
  ...
  ; test "[from_string] Parse empty node with comment and newlines" parse_empty_data_5
  ...
  ; test "[from_string] Invalid Qexp, closed unopened parenthesis" parse_failure_1
